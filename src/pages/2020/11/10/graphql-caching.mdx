---
canonicalURL: "https://ajcwebdev.com/2020/11/10/graphql-caching"
...
tags: [ "GraphQL", "Apollo", "Caching", "urql" ]
image: "https://cdn.hashnode.com/res/hashnode/image/upload/v1654996830003/0w9P9BhIS.jpg"
...
- [Does the GraphQL Specification Address Caching](#does-the-graphql-specification-address-caching)
- [HTTP Caching](#http-caching)
  - [Client Side and Gateway Caches](#client-side-and-gateway-caches)
  - [Freshness and Validation](#freshness-and-validation)
- [GraphQL Caching](#graphql-caching)
  - [Globally Unique IDs](#globally-unique-ids)
  - [Compatibility with Existing APIs](#compatibility-with-existing-apis)
...
- [Apollo Client](#apollo-client)
  ...
  - [Combining Reads and Writes](#combining-reads-and-writes)
  - [Modify Cache](#modify-cache)
- [urql](#urql)
- [micro-graphql-react](#micro-graphql-react)
  - [Import client for global subscriptions to keep cache correct](#import-client-for-global-subscriptions-to-keep-cache-correct)
  - [Sync changes when relevant mutations happen](#sync-changes-when-relevant-mutations-happen)
  - [Cache Resetting](#cache-resetting)
- [Section and Distributed GraphQL](#section-and-distributed-graphql)
...
Only the combination of those two parameters will run a particular procedure on the server. Previous responses to GET requests can be cached and future requests can be routed through the cache. A historical response can be returned if possible.
...
> **Query result trees** represent a way to get trees out of your app data graph. Apollo Client applies two assumptions to cache query result trees.
> * **Same path, same object**: Same query path usually leads to the same piece of information.
> * **Object identifiers when the path isn't enough**: Two results given for the same object identifier represent the same node/piece of information.
>
> Apollo Client will update the query with a new result if any cache node involved in a query result tree is updated.

## Apollo Client

Apollo Client stores the results of its GraphQL queries in a normalized, in-memory cache for responding sparingly to future queries for the same data. Normalization constructs a partial copy of your data graph on your client. The format is optimized for reading and updating the graph as your application changes state. You can configure the cache's behavior for other use cases:
...
`InMemoryCache` can exclude normalization for objects that never change. Objects that are not normalized are embedded within their parent object in the cache. These objects can be accessed via their parent but not directly.
...
This example passes `readQuery` a GraphQL query string with a GraphQL variable.
...
This example fetches a particular item from a todo list.
...
I know nothing about this and this article's length is already out of control but I found one nascent approach that seems worth mentioning. A company called [Section](https://www.section.io/docs/tutorials/distributed-apis/) is trying to build a distributed GraphQL solution. It is fully configurable to address caching challenges without having to maintain a distributed system as the distributed system would be managed by them.

They say that it's simultaneously similar to Apollo Federation but also solving a problem Apollo Federation doesn't solve, so I'm curious how exactly that works. On first look it seems like they are taking the approach of `micro-graphql-react` and giving more cache control back to the developers.
