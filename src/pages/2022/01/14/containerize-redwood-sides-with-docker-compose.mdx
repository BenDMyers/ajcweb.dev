---
canonicalURL: "https://community.redwoodjs.com/t/containerize-redwood-sides-with-docker-compose/2706"
title: "containerize redwood sides with docker compose"
description: "Docker Compose is a tool for multi-container Docker applications. This example containerizes and runs Redwood's Web and API sides with Docker Compose."
publishDate: "2022-01-14"
tags: [ "Redwood", "Docker", "nginx", "GraphQL" ]
image: "https://cdn.hashnode.com/res/hashnode/image/upload/v1647328039260/RHTJO5hiN.jpg"
...
This example containerizes Redwood's Web and API sides into individual containers that can be run with Docker Compose. The code for this example can be found [on my GitHub](https://github.com/ajcwebdev/a-first-look/rw-docker-compose).
...
```bash
yarn create redwood-app rw-docker-compose
cd rw-docker-compose
```
...
rw-docker_api   latest    243369952fa0   57 seconds ago   2.96GB
rw-docker_web   latest    c1610495648c   42 minutes ago   137MB
```
...
a4d2a221278f   rw-docker_web   "/docker-entrypoint.…"   35 seconds ago   Up 34 seconds   80/tcp, 0.0.0.0:8910->8910/tcp   rw-docker-web-1
f5ab7bf289a9   rw-docker_api   "yarn rw serve api -…"   35 seconds ago   Up 34 seconds   0.0.0.0:8911->8911/tcp           rw-docker-api-1
```
...
```bash
gh repo create rw-docker-compose \
  --public \
  --source=. \
  --remote=upstream \
  --push
...
```bash
git remote add origin https://github.com/YOUR_USERNAME/rw-docker-compose.git
...
```bash
docker tag rw-docker-compose_web ghcr.io/YOUR_USERNAME/rw-docker-compose_web
docker tag rw-docker-compose_api ghcr.io/YOUR_USERNAME/rw-docker-compose_api
```
...
```bash
docker push ghcr.io/YOUR_USERNAME/rw-docker-compose_web:latest
docker push ghcr.io/YOUR_USERNAME/rw-docker-compose_api:latest
```
...
```bash
docker pull ghcr.io/YOUR_USERNAME/rw-docker-compose_web:latest
docker pull ghcr.io/YOUR_USERNAME/rw-docker-compose_api:latest
```