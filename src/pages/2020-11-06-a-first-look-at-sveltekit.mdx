...
```bash
npm init svelte ajcwebdev-sveltekit
```
...
> 
> * *Problems? Open an [issue](https://github.com/sveltejs/kit/issues) if none exists already.*
> * *Stuck? Visit us at [svelte.dev/chat](https://svelte.dev/chat)*.
...
`src/routes/index.svelte` contains our JavaScript entry point and includes a single file Svelte component. A `.svelte` file contains three parts:
...
Each page of your app is a Svelte component written in a `.svelte` file. Pages are created by adding files to the `src/routes` directory of your project. The route is determined by the filename. For example, `src/routes/index.svelte` is the root of the site.

Pages are server-rendered to ensure a user's first visit is as fast as possible. A client-side app then takes over if needed. We don't need any JavaScript on this page but we'll load it in development for hot module replacement.
...
![02-home-page-ajcwebdev](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/n7pfnihhcz8uz2kxdgq3.png)

## Modules

The [`$app/env`](https://kit.svelte.dev/docs/modules#$app-env) module provides variables for checking the app's current environment. `browser` is `true` when the app is running in the browser and `false` when running on the server, in development mode, or in production. `dev` is `true` in development mode and `false` in production.
...
<div class="content">
  ...
  <p>
    The <a href="/todos">TODOs</a> page illustrates
...
Enter [localhost:3000/about](http://localhost:3000/about) to see this page.
...
Inside `src/lib/Header` is a `Header.svelte` file which imports `page` from `$app/stores`. Stores are added to the [context](https://svelte.dev/tutorial/context-api) of your root component. They are unique to each request on the server rather than shared between multiple requests handled by the same server simultaneously.
...
For dynamic routes, such as `src/routes/blog/[slug].svelte`, code splitting is not enough to ensure fast startup times. To render the blog post we need to fetch the data for it.

We can't do that until we know what the `slug` is. Lag caused by the browser waiting for the data can be mitigate by adding a `sveltekit:prefetch` attribute for prefetching the data.
...
By default, `yarn build` will generate a Node app that you can run with `node build`. To use a different adapter, install it and update `svelte.config.js` accordingly. We do not need to install the Vercel adapter unless we want to use Vercel specific features. Otherwise `adapter-auto` will have everything needed for deployment
